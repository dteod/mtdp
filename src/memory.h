...
#    define MTDP_DEFINE_STATIC_INSTANCES(Type, Name, Number)                                                                     \
      ...
      {                                                                                                                          \
        ...
        for(unsigned MTDP_MACRO_I_VAR = 0; !MTDP_MACRO_OUT_VAR && MTDP_MACRO_I_VAR < (Number); ++MTDP_MACRO_I_VAR) {             \
          MTDP_MACRO_N_VAR    = MTDP_MACRO_I_VAR / 8;                                                                            \
          MTDP_MACRO_FLAG_VAR = Name##_flags + MTDP_MACRO_N_VAR;                                                                 \
          MTDP_MACRO_MASK_VAR = 1 << (i - MTDP_MACRO_N_VAR * 8);                                                                 \
          if(!(atomic_fetch_or(MTDP_MACRO_FLAG_VAR, MTDP_MACRO_MASK_VAR) & MTDP_MACRO_MASK_VAR)) {                               \
            MTDP_MACRO_OUT_VAR = &Name[MTDP_MACRO_I_VAR];                                                                        \
          }                                                                                                                      \
        }                                                                                                                        \
      ...
      {                                                                                                                          \
        ...
        if(MTDP_MACRO_ARG >= Name && MTDP_MACRO_ARG < Name + (Number)) {                                                         \
          ...
          atomic_fetch_and(Name##_flags + MTDP_MACRO_TMP_VAR / 8, ~(1 << MTDP_MACRO_TMP_VAR % 8));                               \
        }                                                                                                                        \
      }
#  else
#    define MTDP_DEFINE_STATIC_INSTANCE(Type, Name)                                                                              \
      ...
      {                                                                                                                          \
        mtx_init(&Name##_mutex, mtx_plain);                                                                                      \
      }                                                                                                                          \
      MTDP_MEMORY_ACCESS Type* Type##_alloc()                                                                                    \
      {                                                                                                                          \
...
#    define MTDP_DEFINE_STATIC_INSTANCES(Type, Name, Number)                                                                     \
      ...
      {                                                                                                                          \
        ptrdiff_t MTDP_MACRO_TMP_VAR;                                                                                            \
        if(DEFINE_STATIC_INSTANCE_ARG >= Name && MTDP_MACRO_ARG < Name + (Number)) {                                             \
          MTDP_MACRO_TMP_VAR = MTDP_MACRO_ARG - Name;                                                                            \
          mtx_lock(&Name##_mutex);                                                                                               \
          Name##_flags[DEFINE_STATIC_INSTANCE_COUNTER / 8] &= ~(1 << MTDP_MACRO_TMP_VAR % 8);                                    \
          mtx_unlock(&Name##_mutex);                                                                                             \
        }                                                                                                                        \
      }
#  endif
...
#define MTDP_DEFINE_DYNAMIC_INSTANCE(Type)                                                                                       \
  ...
  MTDP_MEMORY_ACCESS void Type##_dealloc(Type* MTDP_MACRO_VAR)                                                                   \
  {                                                                                                                              \
    free(MTDP_MACRO_VAR);                                                                                                        \
  }
